...
WHITE = graphics.create_pen(255, 230, 255)
...
def redraw_display_if_reqd():
    global year, month, day, wd, hour, minute, second, last_second, ms
    ...
    microseconds = utime.ticks_us()
    # Convert microseconds to hundredths of a second
    # There are 1,000,000 microseconds in a second, so divide by 10,000 to get hundredths
    hundredths = (microseconds // 10000) % 100

    microsec = ("{:02d}".format(hundredths))
    #print(microsec)
    ...
    clock = "{:02}:{:02}:{:02}:{:02}".format(hour, minute, second, microsec)

    # calculate text position so that it is centred
    #w = graphics.measure_text(clock, 1)
    #hard lock the lock to 45 witdh so the caluclation puts it more or less center
    w = 45
    x = int(width / 2 - w / 2 + 1)
    y = 2
